...
import flask
from flask import session, request, jsonify, render_template, send_from_directory
...
@app.route("/sw.js")
def service_worker():
    """Serves the service worker from the root path (rather than
    /static/sw.js) so its default scope is '/' and it can control every
    page on the site, not just /static/ assets. Required for the app to
...
def favicon():
    """Browsers request this by default regardless of <link rel="icon">
    tags; serve the real logo instead of a 404."""
    return send_from_directory(app.static_folder, "icons/favicon-32.png")
