...
-----------------------------------------------------------
This is a pure JSON API — every route returns JSON, none render HTML.
The frontend (member dashboard + admin control panel, both static PWA
files) is deployed separately (e.g. Netlify) and talks to this API
cross-origin via fetch() with credentials included, which is why
SESSION_COOKIE_SAMESITE is "None" and CORS is configured with
supports_credentials below — set ALLOWED_ORIGINS to your deployed
frontend URL(s).
...

{
  ...
  "ROLE": "Admin",
  "IS_ACTIVE": true,
  "LAST_ACTIVE": "2026-07-27T00:35:00.000000",  # ISO timestamp, bumped on
                                                  # every authenticated
...
Report endpoint and the admin weekly reports always reflect the funding
history.
"""
import os
...
app.config.update(
    ...
    SESSION_COOKIE_SAMESITE='None'   # Required for the cross-origin (Netlify) frontend
)
...
# carries cookies. Set ALLOWED_ORIGINS on Vercel to your deployed Netlify
# URL(s), comma-separated, e.g.:
#   ALLOWED_ORIGINS=https://pyf-dashboard.netlify.app
ALLOWED_ORIGINS = [
...
def dashboard():
    ...
    frontend code doesn't need a route rename, but it now returns JSON
    like every other endpoint instead of a rendered profile page."""
...
def applications():
    ...
    out = [
        {"app_key": k, **v} for k, v in apps.items()
    ]
...
# ---------------------------------------------------------------------------
# Admin control panel — JSON API
# (The control panel page itself is now a static HTML file on the separate
# frontend deployment, not served from here — this backend only answers
# its fetch() calls.)
# ---------------------------------------------------------------------------
